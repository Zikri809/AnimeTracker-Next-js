...
export default function conversionjikanmal(jikanAnime: any): MalTrackingItem {
  return {
    node: {
      ...
      status: jikanAnime.status
        ? jikanAnime.status.split(' ').map((word: string) => word ? (word[0].toLowerCase() + word.slice(1)) : '').join('_')
        : null,
      ...
      genres: (jikanAnime.genres || []).map((g: any) => ({
        id: g.mal_id,
        name: g.name
      }))
    },
