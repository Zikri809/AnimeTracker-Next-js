...
describe('Jikan Detail Normalize Helper', () => {
  ...
  it('should fall back english title to default title to Anime #id', () => {
    const vm1 = normalizeJikanDetail({ mal_id: 1, title: 'Bebop', title_english: null });
    expect(vm1.displayTitle).toBe('Bebop');

    const vm2 = normalizeJikanDetail({ mal_id: 1, title: null, title_english: null });
  ...
  it('should format trailer URL without breaking autoplay values', () => {
    ...
    expect(vm1.trailerEmbedUrl).toBe('https://youtube.com/embed/123?autoplay=0&mute=0');

    const vm2 = normalizeJikanDetail({
      mal_id: 1,
      trailer: { embed_url: 'https://youtube.com/embed/123' },
    });
    expect(vm2.trailerEmbedUrl).toBe('https://youtube.com/embed/123?autoplay=0&mute=0');
  });
