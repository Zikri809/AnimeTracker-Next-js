...
//const Add_to_watchlist_button = dynamic(() => import('@/ComponentsSelf/add to watchlist button'), { ssr: false })
 ...
 export default function DetailAnime(){
  ...
  useEffect(() => {
    ...
    useEffect(()=>{
        ...
        async function fetchapi(){
            if(router.isReady==true){
                ...
                sessionStorage.setItem('urlparams',String(String(router.query.relation_id ?? '') ?? ''))
            try{


                const response = await fetch('https://api.jikan.moe/v4/anime/'+(String(String(router.query.relation_id ?? '') ?? ''))+'/full')
    ...
    },[String(String(router.query.relation_id ?? '') ?? '')])
    //console.log('id',id)
    return (
        <div className='relative overflow-x-hidden top-0 left-0   m-0   w-screen h-auto overflow-hidden  bg-black text-white font-poppins my-1 antialiased' >
            ...
            <div className='relative top-20 flex pb-38 sm:pb-30 flex-col '>
               ...
               <div className='bg-neutral-900 px-5 py-4 flex flex-col gap-10 '>
                    ...
                    {!isloading && animeinfo && <Add_to_watchlist_button to={router.asPath.split('/')[1]=='morethiseseason' || router.asPath.split('/')[1]=='moreupcoming' || router.asPath.split('/')[1]=='morelastseason'?(router.query.hasOwnProperty('relation_id')?'/'+router.asPath.split('/')[1]+'/'+String(String(router.query.mal_id ?? '') ?? '')+'/relation/'+String(String(router.query.relation_id ?? '') ?? '')+'/tracking':'/'+router.asPath.split('/')[1]+'/'+String(String(router.query.mal_id ?? '') ?? '')+'/tracking'):(router.query.hasOwnProperty('title')?(router.query.hasOwnProperty('relation_id')?'/search/'+router.query.title+'/'+String(String(router.query.mal_id ?? '') ?? '')+'/relation/'+String(String(router.query.relation_id ?? '') ?? '')+'/tracking':'/search/'+router.query.title+'/'+String(String(router.query.mal_id ?? '') ?? '')+'/tracking'):(router.query.hasOwnProperty('mylist_tab')?'/mylist/'+router.query.mylist_tab+'/'+String(String(router.query.mal_id ?? '') ?? '')+'/tracking':'/'+String(String(router.query.mal_id ?? '') ?? '')+'/tracking'))}/>}{/* */}
            </div>
