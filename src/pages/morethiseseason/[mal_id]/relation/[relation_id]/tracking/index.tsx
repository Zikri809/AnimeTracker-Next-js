...
export default function TrackingForm(){
    ...
    return(
        <div className='bg-black w-screen h-[100vh] mb-15 sm:mb-0 overflow-hidden'>
            ...
            <Navbar searchtitle={animeinfo.node.alternative_titles.en ?? animeinfo.node.title }  savebutton={savehandler} />
            <Toaster richColors/>
            <div  key={animeinfo.node.id} className='relative top-25  flex px-5 flex-col'>
                ...
                <div className='flex mb-5  flex-row justify-center w-full border-0 border-blue-500'>
                   ...
                   { animeinfo.node.status=='not_yet_aired'?<Button disabled variant='outline' type="button" ref={(el) => { btnref.current[0] = el; }}  className='bg-black rounded-sm border-gray-400 text-white   focus:bg-black'>Watching</Button>:<Button ref={(el) => { btnref.current[0] = el; }} variant='outline' type="button" onClick={statusbutton} className='bg-black rounded-sm border-gray-400 text-white hover:text-black  focus:bg-black'>Watching</Button>}
                    {animeinfo.node.status=='not_yet_aired' ||animeinfo.node.status=='currently_airing'?<Button disabled type="button" ref={(el) => { btnref.current[1] = el; }}  variant='outline' className='bg-black rounded-sm border-gray-400 text-white '>Completed</Button>: <Button ref={(el) => { btnref.current[1] = el; }}  type="button" variant='outline' onClick={completedclickhandler} className='bg-black rounded-sm border-gray-400 text-white hover:text-black focus:bg-black '>Completed</Button>}
                    <Button variant='outline' ref={(el) => { btnref.current[2] = el; }} type="button"  onClick={statusbutton} className='bg-black rounded-sm border-gray-400 text-white hover:text-black focus:bg-black '>Plan To Watch</Button>
                    <Button variant='outline' ref={(el) => { btnref.current[3] = el; }} type="button"  onClick={statusbutton} className='bg-black rounded-sm border-gray-400 text-white hover:text-black focus:bg-black'>On Hold</Button>
                    <Button variant='outline' ref={(el) => { btnref.current[4] = el; }} type="button"  onClick={statusbutton} className='bg-black rounded-sm border-gray-400 text-white hover:text-black focus:bg-black'>Dropped</Button>
                    </div>
                </div>
