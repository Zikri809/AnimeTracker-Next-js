...
export default function More({ seasonaldata }: MoreProps) {
  ...
  useEffect(() => {
    Promise.resolve().then(() => {
      ...
      if (sessionStorage.getItem('slicearr') !== null) {
      ...
      {isLoading ? (
        ...
        <div className='relative top-18 lg:grid lg:grid-cols-2 w-screen pb-33 sm:pb-15 lg:grid-rows'>
          {animearr.slice(0, currentpagearr).map((element: any) => (
            <Link key={element.node.id} href={'/morelastseason/' + element.node.id}>
              <Horizontalcard
                ref={cardref}
