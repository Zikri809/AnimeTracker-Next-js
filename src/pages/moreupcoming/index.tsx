...
interface MoreUpcomingProps {
  seasonaldata: any[];
  seasoninfo: any;
}

export default function MoreUpcoming({ seasonaldata, seasoninfo }: MoreUpcomingProps) {
  ...
  return (
    <div className='relative top-0 left-0 font-poppins overflow-hidden m-0 w-screen h-auto bg-black text-white ml-1 antialiased'>
      <Morenavabr sectionTitle={'Upcoming Season'} SetAnimeArr={setAnimearr} IsUpdateRef={isupdated} SetpageArr={setpagearr} season={seasoninfo.upcoming_season} year={seasoninfo.upcoming_year} defaultArr={seasonaldata}/>
      {isLoading ? (
        ...
        <div className='relative top-18 lg:grid lg:grid-cols-2 w-screen pb-33 sm:pb-15 lg:grid-rows'>
          {animearr.slice(0, currentpagearr).map((element: any) => (
            <Link key={element.node.id} href={'/moreupcoming/' + element.node.id}>
              <Horizontalcard
                ref={cardref}
