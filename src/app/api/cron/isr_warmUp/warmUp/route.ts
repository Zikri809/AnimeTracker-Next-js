...
export async function GET(_request: NextRequest) {
  // Never derive outbound fetch targets from the incoming Host header.
  // Warm-up requests must use trusted app configuration only.
  const baseUrl = getBaseUrl();

  const static_routes = ["morethiseseason", "morelastseason", "moreupcoming"];
  ...
  const route_objects: WarmUpRoute[] = [
    ...static_routes,
    ...dynamic_routes,
  ].map((value) => ({
    route: value,
